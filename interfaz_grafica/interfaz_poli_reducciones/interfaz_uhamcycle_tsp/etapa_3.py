...
# que TSP-DEC es NP-Completo. Esto se debe a la aplicación del 
...
class Etapa3(Etapa):
    ...
    def lanzar_etapa(self):
        ...
        self.panel_3.pack(padx=10, pady=10, fill="both",expand=True)

        panel_titulo = ctk.CTkFrame(self.panel_3, fg_color="#6889B1")
        ...
        panel_subtitulo = ctk.CTkFrame(self.panel_3, fg_color="#A4B6CD")
        panel_subtitulo.pack(padx=30, pady=(50,10))

        label = ctk.CTkLabel(panel_subtitulo, text="TSP-DEC es NP-Completo", fg_color="#A4B6CD",
                                font=ctk.CTkFont(size=14,weight="bold"))
        label.pack(padx=30, pady=10)

        panel_texto = ctk.CTkFrame(self.panel_3)
        ...
        texto = "En nuestro caso, aplicamos el teorema con L = UHAMCYCLE (ya sabemos que \nes NP-Completo) " \
        "y L'= TSP-DEC, así que concluimos que TSP-DEC es \nNP-Completo."

        label = label = ctk.CTkLabel(panel_texto, text=texto)
        label.pack(padx=10, pady=10, side = "top")

        panel_botones_6 = ctk.CTkFrame(self.panel_3)
        panel_botones_6.pack(padx=10, pady=(0,10), fill=tk.Y, side='bottom')

        
        boton_anterior = ctk.CTkButton(panel_botones_6, text="Anterior", command=lambda:self.gestor_etapas.anterior(2))
        boton_anterior.grid(row=0, column=0, padx=(10,20), pady=10)

        boton_siguiente = ctk.CTkButton(panel_botones_6, text="Fin", command=lambda:self.gestor_etapas.siguiente(4))
