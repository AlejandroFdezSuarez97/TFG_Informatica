...
class GestorEtapas():
    ...
    def cerrar_simulacion(self):
        ...
        grafoAlmacenado = False

        # Guadamos en la BD la reduccion realizada. Usaremos como primary key el numero actual de reducciones
        # gaurdadas en la BD
        ...
        if id_formula==-1: # Si la formula no esta en tabla_sat
            ...
            Controlador.get_unica_instancia().get_base_datos().get_tabla_sat().anadir_fila_tabla_sat(id_sat, self.get_formula(), self.get_literales_formula(), self.get_clausulas_formula(), self.get_num_literales_formula(), self.get_num_clausulas_formula(), self.get_formula_is_satisfacible(),self.etapa_5.get_lista_valores_literales() )
        else:
            id_sat = id_formula
            formulaAlmacenada = True


        # ---------------------------- Almacenar Grafo en tabla_hampath
        
        # Si la formula introducida no era satisfacible, almacenamos un grafo que no contiene un camino hamiltoniano
        if self.lista_informacion_calculada[3]==None: 
            camino_hamiltoniano = 0 # El cero significa que no contiene camino hamiltoniano
            lista_grafo_1 = self.lista_informacion_calculada[0]
            lista_grafo_2 = self.lista_informacion_calculada[1]
        else:
            camino_hamiltoniano = 1 # El uno significa que si contiene camino hamiltoniano
            lista_grafo_1 = self.lista_informacion_calculada[3]
            lista_grafo_2 = self.lista_informacion_calculada[4]

        # extraigo toda la informacion que quiero guardar
        grafo_1 = lista_grafo_1[0]
        pos_grafo_1 = lista_grafo_1[1]
        etiquetas_grafo_1 = lista_grafo_1[2]
        color_map_grafo_1 = lista_grafo_1[3]
        edge_color_grafo_1 = lista_grafo_1[4]
        posicionesNodos_ST_grafo_1 = lista_grafo_1[5]

        grafo_2 = lista_grafo_2[0]
        pos_grafo_2 = lista_grafo_2[1]
        etiquetas_grafo_2 = lista_grafo_2[2]
        color_map_grafo_2 = lista_grafo_2[3]
        edge_color_grafo_2 = lista_grafo_2[4]
        posicionesNodos_ST_grafo_2 = lista_grafo_2[5]
        
        # Compruebo si el grafo está guardado en la BD
        id_hampath = Controlador.get_unica_instancia().get_base_datos().get_tabla_hampath().is_grafo_in_tabla_hampath(grafo_1, grafo_2)

        if id_hampath==-1:
            id_hampath = id
            Controlador.get_unica_instancia().get_base_datos().get_tabla_hampath().anadir_fila_tabla_hampath(id_hampath, camino_hamiltoniano,
                                                                                                     grafo_1,pos_grafo_1, etiquetas_grafo_1, color_map_grafo_1, edge_color_grafo_1, posicionesNodos_ST_grafo_1,
                                                                                                     grafo_2,pos_grafo_2, etiquetas_grafo_2, color_map_grafo_2, edge_color_grafo_2, posicionesNodos_ST_grafo_2 )
        else:
            grafoAlmacenado = True

        
        # No almaceno en tabla_simulador si la formula y el grafo ya estaban gurdados
        # Porque eso significa que ya habiamos hecho esa reduccion antes.
        if (not formulaAlmacenada) or (not grafoAlmacenado):
            # Inserto en la tabla_simulacion
            Controlador.get_unica_instancia().get_base_datos().get_tabla_simulador().anadir_fila_tabla_simulador(id, id_sat, id_hampath, "SAT", "HAMPATH")
