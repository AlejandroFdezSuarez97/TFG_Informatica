...
class GestorEtapas():
    ...
    def __init__(self, ventana, lista_resultados_previos):
        ...
        # Lista que almacenará toda la info del grafo hamcycle final
        self.grafo_final = []

        self.grafo_inicial_con_ciclo = False
    ...
    def get_grafo_final(self):
        return self.grafo_final
    
    def set_grafo_final(self, info_Grafo_final):
        self.grafo_final = info_Grafo_final

    def get_grafo_inicial_con_ciclo(self):
        return self.grafo_inicial_con_ciclo

    def set_grafo_inicial_con_ciclo(self, bool):
        self.grafo_inicial_con_ciclo = bool
    ...
    def cerrar_simulacion(self):
        ...
        (id_grafoHampath, grafo, pos, colorMap, edgeMap, grosorArcos, cicloHamiltoniano) = self.get_grafo_final()

        # Comprobar si el grafo final ya está almacenado en la tabla_hamcyle
        id_encontrado = Controlador.get_unica_instancia().get_base_datos().get_tabla_hamcycle().is_grafo_in_tabla_hamcycle(id_grafoHampath, grafo)
            
        # Si el hamcycle no esta almacenado en la BD
        if id_encontrado==-1:
            Controlador.get_unica_instancia().get_base_datos().get_tabla_hamcycle().anadir_fila_tabla_hamcycle(numReduccionesBD, id_grafoHampath, grafo, pos, colorMap, edgeMap, grosorArcos,cicloHamiltoniano)
            
            # Si no he usado uno previo para almacenarlo
            if id_grafoHampath == -1:
                # Guardo en tabla_simulador la reduccion 
                Controlador.get_unica_instancia().get_base_datos().get_tabla_simulador().anadir_fila_tabla_simulador(numReduccionesBD,0, numReduccionesBD," ", "HAMCYCLE")
            else:
                # En este caso, hemos usado un grafo que estaba almacenado en la BD
                Controlador.get_unica_instancia().get_base_datos().get_tabla_simulador().anadir_fila_tabla_simulador(numReduccionesBD, id_grafoHampath, numReduccionesBD,"HAMPATH", "HAMCYCLE")
        
        # Si el hamcycle ya estaba almacenado, no hago nada
