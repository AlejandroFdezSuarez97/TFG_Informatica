...
class Etapa1(Etapa):
        ...
        if self.gestor_etapas.get_grafo_inicial() == None:
            ...
            if self.etapa_realizada == True:
                ...
                # Opción 2: partir del grafo hamiltoniano previo
                    ...
                    if (self.gestor_etapas.get_etapa(0).get_fila_bd()[1])==1:
                        texto += "Con esto, ya hemos obtenido el grafo G', que contiene un ciclo \nhamiltoniano.\nSe ve claramente que G ∈ HAMPATH"\
                                " si y sólo si G' ∈ HAMCYCLE."
    ...
    def crear_panel_grafo_nuevo(self, grafo):
        ...
        # Establezco el grafo_Hamcycle_final

        if self.gestor_etapas.get_grafo_nuevo_con_camino():
            cicloHamiltoniano = 1
            #print('Guardo el grafo hamcycle indicando que SI tiene camino hamiltoniano')
        else:
            cicloHamiltoniano = 0
            #print('Guardo el grafo hamcycle indicando que NO tiene camino hamiltoniano')

        grafo_final = [-1, grafo, {}, color_map, edge_colors,grosor_arcos, cicloHamiltoniano]
    ...
    def crear_panel_grafo_previo(self, tercer_grafo):
        ...
        fila_bd = self.gestor_etapas.get_etapa(0).get_fila_bd()
        id_grafoHampath = fila_bd[0]

        # 1º Grafo
        (grafo, pos, etiquetas, color_map, edge_colors, posiciones_s_t) = fila_bd[2:8]
        ...
        if pos=={}:
            ...
            # 2º Grafo
            (grafo, pos, etiquetas, color_map, edge_colors, posiciones_s_t) = fila_bd[8:14]
            ...
            grafo_final = [id_grafoHampath, tercer_grafo, pos_tercer_grafo, color_map, edge_colors,grosor_arcos, cicloHamiltoniano]
            self.gestor_etapas.set_grafo_final(grafo_final)
