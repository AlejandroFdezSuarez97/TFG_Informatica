...
class PanelPrincipal(Panel):
    ...
    def construir_grafo(self, aristas_a_actualizar):
        ...
        for id_nodo in Controlador.get_unica_instancia().get_lista_id_nodos():
            ...
            if id_nodo==0 :# SAT3cnf
                pos_x = 1
                pos_y = 1
            elif id_nodo==1 : #HamPath
                pos_x = 1
                pos_y = 2
            elif id_nodo==2 : # Hamcycle
                pos_x = 1.5
                pos_y = 4
            elif id_nodo==3 : #uhamcycle
                pos_x = 2
                pos_y = 3
            elif id_nodo==4 : #TSP-DEC
                pos_x = 2.5
                pos_y = 2
            elif id_nodo==5 : #Subsetsum
                pos_x = 2
                pos_y = 1
            elif id_nodo==6: # mochila
                pos_x = 1.5
                pos_y = 2
            ...
            contadorDistribuirNuevosNodos = 0
            numNodosVacios = 16 - num_nodos

            # ------- Anadimos nuevos nodos que no son problemas
            ...
            if id_nodo == num_nodos - 1:
                for i in range (id_nodo + 1, num_nodos + numNodosVacios):
                    g.add_node(i) 

                    etiquetas[i] = ""
                    
                    if contadorDistribuirNuevosNodos < 1:
                        pos_x = 1
                        pos_y = 3
                    elif contadorDistribuirNuevosNodos < 2:
                        pos_x = 1
                        pos_y = 4
                    elif contadorDistribuirNuevosNodos < 3:
                        pos_x = 1.5
                        pos_y = 3
                    elif contadorDistribuirNuevosNodos < 4:
                        pos_x = 1.5
                        pos_y = 1
                    elif contadorDistribuirNuevosNodos < 5:
                        pos_x = 2
                        pos_y = 4
                    elif contadorDistribuirNuevosNodos < 6:
                        pos_x = 2
                        pos_y = 2
                    elif contadorDistribuirNuevosNodos < 7:
                        pos_x = 2.5
                        pos_y = 4
                    elif contadorDistribuirNuevosNodos < 8:
                        pos_x = 2.5
                        pos_y = 3
                    elif contadorDistribuirNuevosNodos < 9:
                        pos_x = 2.5
                        pos_y = 1
                    else:
                        pos_x = 2.5
                    ...
                    contadorDistribuirNuevosNodos += 1
                    pos[i] = (pos_x,pos_y)
        ...
        for i in range(num_nodos, num_nodos + numNodosVacios-3):
            nodo_a = random.randrange(num_nodos, num_nodos + numNodosVacios, 1)
            nodo_b = random.randrange(num_nodos, num_nodos + numNodosVacios, 1)
        ...
        nx.draw_networkx(g, ax=axis, node_shape="s", with_labels=False, edge_color=edge_colors,
                            node_color=color_map, width=grosor_arcos, pos=pos, node_size=840, alpha=0.8)
    ...
    def crear_ventana_emergente(self, texto, label_info, error):
        ...
        ventana_emergente.iconbitmap('interfaz_grafica/interfaz_app/img/simulador.ico')
    ...
    def iniciar_simulacion(self):
        ...
        if (self.problema_1 == "") and (self.problema_2 == ""):
            ...
            if Controlador.get_unica_instancia().get_num_simulaciones() == 1:
                ...
                aristas = []
                aristas = Controlador.get_unica_instancia().buscar_aristas(self.problema_1, self.problema_2, aristas)
    ...
    def crear_ventana_inicio_fin_simulacion(self, texto, nombre_nodos_inicio, lista_texto, nombre_nodos_fin, realizada):
        ...
        ventana_fin_reduccion.iconbitmap('interfaz_grafica/interfaz_app/img/simulador.ico')
