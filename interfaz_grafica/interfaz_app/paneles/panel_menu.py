...
class PanelMenu():
    ...
    def crear_panel_menu(self):
        ...
        bg_image = ctk.CTkImage(Image.open("interfaz_grafica/interfaz_app/img/logo_girado.png"), size=(190, 190))
        ...
        boton_bd = ctk.CTkButton(panel_menu, text="Base de Datos", fg_color="#E6E6FA", hover_color="#CCCCFF",
                                     bg_color="#263b5b",text_color=("#000000","#FFFFFF"), command=lambda:self.mostrar_bd())
        boton_bd.grid(row=8, column=0, padx=20)

        boton_salir = ctk.CTkButton(panel_menu, text="Salir", fg_color="red", hover_color="#D22121",
                                     bg_color="#263b5b", command=lambda:exit())
        boton_salir.grid(row=9, column=0, padx=20, pady=20)
    ...
    def exit(self):
        ...
        Controlador.get_unica_instancia().get_base_datos().get_conexion().close()
        sys.exit()
    ...
    # Funcion que muestra una ventana con las tablas que mostrar en la base de datos
    def mostrar_bd(self):
        ventana_mostrar_bd = VentanaPopUp(self.ventana)

        ventana_mostrar_bd.geometry("")
        ventana_mostrar_bd.iconbitmap('interfaz_grafica/interfaz_app/img/logo_bd.ico')
        ventana_mostrar_bd.title("Mostrar BD")
        ventana_mostrar_bd.resizable(1,1)

        label = ctk.CTkLabel(ventana_mostrar_bd, fg_color=("#A3B5CC","gray20"), text="¿Qué quieres hacer?",
                                font=ctk.CTkFont(size=15, weight="bold"))
        label.pack(padx=10, pady=10, side="top", fill=ctk.X)

        panel_izqdo = ctk.CTkFrame(ventana_mostrar_bd)
        panel_izqdo.pack(fill="both", expand="True", padx=10, pady=10, side="left")

        panel_derecho = ctk.CTkFrame(ventana_mostrar_bd)
        panel_derecho.pack(fill="both", expand="True", padx=(0,10), pady=10, side="right")

        boton_ver_bd  = ctk.CTkButton(panel_izqdo, text="Ver contenido de BD", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=lambda:self.ver_bd(ventana_mostrar_bd))
        boton_ver_bd.pack(padx=10, pady=10)

        boton_vaciar_bd = ctk.CTkButton(panel_derecho,text="Vaciar Completamente BD", fg_color="#DC143C", 
                                        hover_color="#FF0000", width=200, command=lambda:self.vaciar_bd(ventana_mostrar_bd))
        boton_vaciar_bd.pack(padx=10, pady=10)

        ventana_mostrar_bd.center()
    
    # Implementa los botones para mostrar el contenido de la base de datos
    def ver_bd(self, ventana):
        ...
        label = ctk.CTkLabel(ventana_mostrar_bd, fg_color=("#A3B5CC","gray20"), text="Selecciona qué tabla quieres visualizar:",
                                font=ctk.CTkFont(size=15, weight="bold"))
        label.pack(padx=10, pady=10, side="top", fill=ctk.X)
        ...
        # botones para ver cada tabla de la bbdd
        boton_tabla_simulador = ctk.CTkButton(panel_izqdo, text="tabla Simulador", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=lambda:Controlador.get_unica_instancia().get_base_datos().get_tabla_simulador().mostrar_tabla_simulador(ventana_mostrar_bd))
        boton_tabla_simulador.pack(padx=10, pady=10)

        boton_tabla_hampath = ctk.CTkButton(panel_izqdo, text="tabla Hampath", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=lambda:Controlador.get_unica_instancia().get_base_datos().get_tabla_hampath().mostrar_tabla_hampath(ventana_mostrar_bd))
        boton_tabla_hampath.pack(padx=10, pady=20)

        boton_tabla_uhamcycle = ctk.CTkButton(panel_izqdo, text="tabla Uhamcycle", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=lambda:Controlador.get_unica_instancia().get_base_datos().get_tabla_uhamcycle().mostrar_tabla_uhamcycle(ventana_mostrar_bd))
        boton_tabla_uhamcycle.pack(padx=10, pady=20)

        boton_tabla_subset = ctk.CTkButton(panel_izqdo, text="tabla Subset", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=lambda:Controlador.get_unica_instancia().get_base_datos().get_tabla_subset().mostrar_tabla_subset(ventana_mostrar_bd))
        boton_tabla_subset.pack(padx=10, pady=20)


        boton_tabla_sat = ctk.CTkButton(panel_derecho, text="tabla Sat", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=lambda:Controlador.get_unica_instancia().get_base_datos().get_tabla_sat().mostrar_tabla_sat(ventana_mostrar_bd))
        boton_tabla_sat.pack(padx=10, pady=10)

        boton_tabla_hamcycle = ctk.CTkButton(panel_derecho, text="tabla Hamcycle", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=lambda:Controlador.get_unica_instancia().get_base_datos().get_tabla_hamcycle().mostrar_tabla_hamcycle(ventana_mostrar_bd))
        boton_tabla_hamcycle.pack(padx=10, pady=20)

        boton_tabla_tsp = ctk.CTkButton(panel_derecho, text="tabla TSP-DEC", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=lambda:Controlador.get_unica_instancia().get_base_datos().get_tabla_tsp().mostrar_tabla_tsp(ventana_mostrar_bd))
        boton_tabla_tsp.pack(padx=10, pady=20)

        boton_tabla_mochila = ctk.CTkButton(panel_derecho, text="tabla Mochila", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=lambda:Controlador.get_unica_instancia().get_base_datos().get_tabla_mochila().mostrar_tabla_mochila(ventana_mostrar_bd))
        boton_tabla_mochila.pack(padx=10, pady=20)

        ventana_mostrar_bd.center()

    # Implementa una funcion que elimina completamente el contenido de la bd
    def vaciar_bd(self, ventana):
        # Eliminamos la anterior ventana
        ventana.destroy()

        # Creamos un panel para volver a preguntar si queremos eliminar completamente la BD
        ventana_advertencia = VentanaPopUp(self.ventana)

        ventana_advertencia.geometry("")
        ventana_advertencia.iconbitmap('interfaz_grafica/interfaz_app/img/logo_bd.ico')
        ventana_advertencia.title("ADVERTENCIA")
        ventana_advertencia.resizable(1,1)

        label = ctk.CTkLabel(ventana_advertencia, fg_color=("#A3B5CC","gray20"), text="¿Estás seguro de que quieres vaciar completamente la Base de Datos?",
                                font=ctk.CTkFont(size=15, weight="bold"))
        label.pack(padx=10, pady=10, side="top", fill=ctk.X)

        panel = ctk.CTkFrame(ventana_advertencia, corner_radius=0) 
        panel.pack(fill = "both", expand=True)

        panel_imagen = ctk.CTkFrame(panel)
        panel_imagen.pack(fill="both", expand="True", padx=10, pady=10, side="top")

        panel_botones = ctk.CTkFrame(panel)
        panel_botones.pack(fill="both", expand="True", padx=10, pady=10, side="bottom")

        panel_boton_izq = ctk.CTkFrame(panel_botones)
        panel_boton_izq.pack(fill="both", expand="True", padx=10, pady=10, side="left")

        panel_boton_dch = ctk.CTkFrame(panel_botones)
        panel_boton_dch.pack(fill="both", expand="True", padx=10, pady=10, side="right")

        # Imagen aviso
        bg_image = ctk.CTkImage(Image.open("interfaz_grafica/interfaz_app/img/help.png"), size=(150, 150))
        label = ctk.CTkLabel(panel_imagen, image=bg_image, text="")
        label.pack(fill = "both", expand=True)

        # Botones
        boton_vaciar  = ctk.CTkButton(panel_boton_izq, text="VACIAR CONTENIDO", fg_color="#DC143C", 
                                        hover_color="#FF0000", width=200, command=lambda:self.vaciar(ventana_advertencia))
        boton_vaciar.pack(fill = "both", expand=True)

        boton_cancelar = ctk.CTkButton(panel_boton_dch,text="CANCELAR", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=lambda:self.cancelar(ventana_advertencia))
        boton_cancelar.pack(fill = "both", expand=True)

        ventana_advertencia.center()

    # Implementa el vaciado completo de la base de datos
    def vaciar(self,ventana):
        ventana.destroy()
        Controlador.get_unica_instancia().get_base_datos().vaciar_bd()
        Controlador.get_unica_instancia().actualizar_numReduccionesBD()

        ventana_bd_vacia = VentanaPopUp(self.ventana)

        ventana_bd_vacia.geometry("")
        ventana_bd_vacia.iconbitmap('interfaz_grafica/interfaz_app/img/logo_bd.ico')
        ventana_bd_vacia.title("Información")
        ventana_bd_vacia.resizable(1,1)

        label = ctk.CTkLabel(ventana_bd_vacia, fg_color=("#A3B5CC","gray20"), text="Se ha vaciado con éxito la base de datos",
                                font=ctk.CTkFont(size=15, weight="bold"))
        label.pack(padx=10, pady=10, side="top", fill=ctk.X)

        panel = ctk.CTkFrame(ventana_bd_vacia)
        panel.pack(fill="both", expand="True", padx=10, pady=10, side="bottom")

        boton_aceptar  = ctk.CTkButton(panel, text="Aceptar", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=ventana_bd_vacia.destroy)
        boton_aceptar.pack(padx=10, pady=10)

        ventana_bd_vacia.center()

    # Cancela la operacion de vaciado de la base de datos
    def cancelar(self, ventana):
        ventana.destroy()

        ventana_cancelar = VentanaPopUp(self.ventana)

        ventana_cancelar.geometry("")
        ventana_cancelar.iconbitmap('interfaz_grafica/interfaz_app/img/logo_bd.ico')
        ventana_cancelar.title("Información")
        ventana_cancelar.resizable(1,1)

        label = ctk.CTkLabel(ventana_cancelar, fg_color=("#A3B5CC","gray20"), text="Se ha cancelado el proceso de eliminación de la base de datos",
                                font=ctk.CTkFont(size=15, weight="bold"))
        label.pack(padx=10, pady=10, side="top", fill=ctk.X)

        panel = ctk.CTkFrame(ventana_cancelar)
        panel.pack(fill="both", expand="True", padx=10, pady=10, side="bottom")

        boton_aceptar  = ctk.CTkButton(panel, text="Aceptar", fg_color=("#70AB94","#4D8871"), 
                                        hover_color=("#7FB9A2","#52987C"), width=200, command=ventana_cancelar.destroy)
        boton_aceptar.pack(padx=10, pady=10)

        ventana_cancelar.center()