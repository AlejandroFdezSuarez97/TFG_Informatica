...
##############################################################################
# Clase que engloba el tipo de tabla_Uhamcycle.
##############################################################################

class Tabla_uhamycle(Tabla):
    ...
    # Funcion para crear/cargar la tabla_uhamcycle
    def cargar_tabla_uhamcycle(self):
        self.cursor.execute('''
                            CREATE TABLE IF NOT EXISTS tabla_uhamcycle (
    ...
    # Funcion que almacena una entrada en la tabla_uhamcycle
    # Debo serializar la mayoria de sus parametro, por lo que debo tener cuidado a la hora de 
    # sacarlos de la BD.
    def anadir_fila_tabla_uhamcycle(self, id, grafo, colorMap, ciclo_hamiltoniano):

        sql = "INSERT INTO tabla_uhamcycle (id, Grafo, ColorMap, CicloHamiltoniano) VALUES (?, ?, ?, ?)"    
        ...
        self.cursor.execute(sql, (id, grafo_serializado, colorMap_serializado,ciclo_hamiltoniano))
    ...
    # Funcion que sirve para extaer una fila de la tabla_uhamcycle
    def extaer_fila_tabla_uhamcycle_por_id(self, id):
        sql =  "SELECT * FROM tabla_uhamcycle WHERE id = ?"
        ...
        if fila is not None:
            ...
            filaSolucion.append(fila[3])                   # CicloHamiltoniano
    ...
    # Obtener lista de todos los grafos almacenados en tabla_uhamcycle
    def get_todos_grafos_tabla_uhamcycle(self):
        sql = "SELECT Grafo FROM tabla_uhamcycle"
    ...
    # Devuelve una lista con todas las filas(entradas) de la tabla uhamcycle
    def get_todas_filas_tabla_uhamcycle(self):
        sql = "SELECT * FROM tabla_uhamcycle"
        self.cursor.execute(sql)
        ...
        for fila in filas:
            ...
            nueva_fila.append(fila[3])                   # CicloHamiltoniano
    ...
    def is_grafo_in_tabla_uhamcycle(self, g):

        id = -1

        # buscar si existe alguna fila que contenga al grafo y coincida el id_hampath
        sql = "SELECT id, Grafo FROM tabla_uhamcycle WHERE Grafo = ?"
    ...
    # Funcion que imprime toda la tabla_uhamcycle en la terminal
    def imprime_tabla(self):
        
        self.cursor.execute("SELECT * FROM tabla_uhamcycle")
        filas = self.cursor.fetchall()
    ...
    # en formato grafico la tabla_uhamcycle
    def mostrar_tabla_uhamcycle(self, ventana_antigua):
        ...
        if len(entradas)==0:
            ...
            label = ctk.CTkLabel(panel, text='La tabla uhamcycle se encuentra actualemente vacía.')
