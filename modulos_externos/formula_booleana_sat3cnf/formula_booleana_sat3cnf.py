...
class FormulaBooleanaSat3cnf():
    ...
    def __init__(self, ventana, panel_base, nombre_reduccion):
        ...
        # Para saber si escojo el introducir una nueva formula o una de la BD
        self.nueva_formula = True
    ...
    def mostrar_formula_booleana(self):
        ...
        # Obtengo una lista con todas las formulas almacenadas en la BD
        # -----------------------------------------------------------------------------------------
        listaFormulasBD = Controlador.get_unica_instancia().get_base_datos().get_tabla_sat().get_todas_formulas_tabla_sat()
        listaFormulasBD.append("--- Insertar nueva formula ---")

        self.formula = tk.StringVar()
        
        combobox = ctk.CTkComboBox(panel_interior, values=listaFormulasBD, command=lambda formula: self.escogerFormulaBooleana(formula, panel_interior), width=300)
        combobox.grid(row=2, column=1, padx=10, pady=10, columnspan=2)

        # -----------------------------------------------------------------------------------------

        boton_masInfo = ctk.CTkButton(panel_interior, text="Más info", fg_color="#6B8399", hover_color="#7A96B0",
                                            command=lambda:self.mostrar_info_formato_sat3cnf(), width=20)
        boton_masInfo.grid(row=3, column=0, padx=10, pady=10) 
        ...
        boton_limpiar = ctk.CTkButton(panel_interior, text="Limpiar", fg_color="#D79E12", hover_color="#F0B72D",
                                        command=lambda:self.limpiar_texto(panel_interior))
    ...
    def limpiar_texto(self, panel_interior):
    ...
    def escogerFormulaBooleana(self, formula : str, panel_interior):
        self.nueva_formula = False
        if formula == "--- Insertar nueva formula ---":
            self.nueva_formula = True
